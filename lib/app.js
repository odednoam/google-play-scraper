...
const localeStrings = require('./localeStrings');
...
function parseFields ($) {
  const lang = $("html").attr("lang").split('_')[0];
  ...
  const iapDetails = additionalInfo.find("div.title:contains(" + localeStrings.inAppProducts[lang] + ")+div.content").text().trim();
  let iapMinPrice, iapMaxPrice;
  if (iapDetails) {
    let m = /^(\S+) - (\S+)/.exec(iapDetails);
    if (m) {
      iapMinPrice = m[1];
      iapMaxPrice = m[2];
    }
  ...
  let similarApps = [];
  $('div.details-section.recommendation .card[data-docid]').each(function() {
      similarApps.push($(this).data('docid'));
  });
